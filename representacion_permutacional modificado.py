...
# Nueva función de cálculo de fitness con penalización por restricción
def calcular_fitness(cromosoma):
    asignaciones = decodificar_cromosoma(cromosoma)
    
    # Verificar la restricción: No todos los alumnos con notas < 11 en el mismo examen
    for examen in ['A', 'B', 'C']:
        notas_examen = [notas[i] for i in asignaciones[examen]]
        if sum([1 for nota in notas_examen if nota < 11]) == len(notas_examen):  # Todos los alumnos con nota < 11
            return -10000  # Penalización fuerte por violar la restricción

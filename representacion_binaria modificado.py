...
def calcular_fitness(cromosoma):
    ...
    for examen in ['A', 'B', 'C']:
        ...
        # Promedio de las notas del examen
        promedios[examen] = np.mean(notas_examen)
        
        # Varianza de las notas del examen
        varianzas[examen] = np.var(notas_examen)
        
        # Premiar diversidad: si la varianza es alta, se premia con un pequeño bono
        if varianzas[examen] > 4:  # Umbral para premiar diversidad
            diversidad_bonus += 0.1
    
    # Penalizar alta varianza (menor fitness)
    penalizacion_varianza = sum(varianzas.values()) / 3  # Promedio de las varianzas
    
    # Devolvemos el fitness, combinando penalización por varianza y premio por diversidad
    return -(penalizacion_varianza - diversidad_bonus)

...
# Mutación Gaussiana - Actividad 3
def mutacion_gaussiana(cromosoma, sigma=0.1):
    cromosoma_mutado = cromosoma.copy()
    
    for i in range(39):
        if random.random() < 0.1:  # Probabilidad de mutación para cada alumno
            idx = i * 3
            # Aplicamos mutación gaussiana a los pesos
            nuevos_pesos = [cromosoma_mutado[idx + j] + random.gauss(0, sigma) for j in range(3)]
            
            # Aseguramos que los nuevos pesos estén normalizados (sumen a 1)
            suma = sum(nuevos_pesos)
            if suma > 0:
                nuevos_pesos = [peso / suma for peso in nuevos_pesos]
            else:
                nuevos_pesos = [1/3, 1/3, 1/3]  # Para evitar dividir entre 0
            
            cromosoma_mutado[idx:idx+3] = nuevos_pesos  # Actualizamos el cromosoma con los nuevos pesos
